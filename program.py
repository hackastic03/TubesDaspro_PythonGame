...
def sumJin(): # Fungsi untuk menghitung banyaknya jin di file user.csv
    n = 0
...
(2) Pembangun - Bertugas membandung candi""")
            while True:
                jenisJin = int(input("Masukkan nomor jenis jin yang dipanggil: ")) # Input nomor jin yang ingin dipanggil
                if jenisJin == 1:
            ...
            if rolejin == "Pembangun" or rolejin == "Pengumpul":
                validasiUsernameJin = True # Inisialisasi kondisi awal
                while validasiUsernameJin:
                    daftarUserJin = input("Masukkan username jin: ") # Input username jin yang ingin ditambah ke file user.csv
                    if daftarUserJin == "-":
                ...
                while not validasiUsernameJin:
                    daftarPassJin = input("Masukkan password jin: ") # Input password untuk akun jin tersebut
                    if len(daftarPassJin) >= 5 and len(daftarPassJin) <= 25:
                        for j in range(1, data.elemUser[1]):
                            if data.user[j][0] == "-" and data.user[j][1] == "-" and data.user[j][2] == "-": # Menambahkan jin di baris yang kosong dan menyimpannya di file user.csv
                                data.user[j][0], data.user[j][1], data.user[j][2] = daftarUserJin, daftarPassJin, rolejin
                                validasiUsernameJin = True
                                print("Jin berhasil di-summon.")
                                break
                        else:
                            data.user = app(data.user, data.elemUser[1], [daftarUserJin, daftarPassJin, rolejin])[0] # Menambah jin di baris baru dan menyimpannya di file user.csv
            ...
            print("Jumlah jin sudah maksimal!") # Keadaan ketika jumlah jin sudah maksimal
    else:
        print("Anda tidak punya akses!") # Keadaan ketika bukan Bandung Bondowoso yang mengakses
...
def hapusjin():
    if data.roleIn == "bandung_bondowoso":
        ...
        if validasiUsernameJin and sumJin() > 3:
            deleteJin = input("Masukkan username jin : ") # Input username jin yang ingin dihapus
            for i in range(1, data.elemUser[1]):
                    ...
                    if checkDeleteJin == "Y" or checkDeleteJin == "y":
                        for j in range(data.elemBahan[1]):
                            if deleteJin == data.bahan[j][0]:
                                data.bahan[j][0], data.bahan[j][1], data.bahan[j][2] = "-", "-", "[0, 0, 0]" # Mereset data bahan di file bahan_bangunan.csv menjadi kosong
                        for j in range(data.elemCandi[1]):
                            if deleteJin == data.candi[j][1]:
                                data.candi[j][1], data.candi[j][2], data.candi[j][3], data.candi[j][4] = "-", 0, 0, 0 # Mereset data candi di file candi.csv menajdi kosong
                        print("Jin telah berhasil dihapus dari alam gaib.")
                        data.user[i][0], data.user[i][1], data.user[i][2] = "-", "-", "-" # Menghapus data jin tersebut di file user.csv
                        break
                        ...
                        break
                elif deleteJin == "Bondowoso" or deleteJin == "Roro": # Keadaan ketika ingin menghapus Bandung Bondowoso atau Roro
                    print("Anda tidak bisa menghapus user!")
                    break
            else:
                print("Tidak ada jin dengan username tersebut.") # Keadaan ketika input username jin yang ingin dihapus tidak terdapat di file user.csv
        else:
            print("Anda tidak memiliki jin!") # Keadaan ketika data user.csv masih kosong 
    else:
        print("Anda tidak bisa menghapus jin!") # Keadaan ketika bukan Bandung Bondowoso yang mengakses


#F05 Ubah Jin
def ubahjin():
    if data.roleIn == "bandung_bondowoso":
        kondisi = True # Inisialisasi kondisi awal
        if sumJin() > 3:
            daftarUserJin = input("Masukkan username jin: ") # Input username jin yang ingin diubah tipenya
            for i in range(1, data.elemUser[1]):
                if daftarUserJin == data.user[i][0]:
            ...
            while not kondisi: # Keadaan ketika input username jin yang ingin diubah tipenya tidak terdapat di data user.csv
                print("Tidak ada jin dengan username tersebut")
                break
        else:
            print("Anda tidak memiliki jin!") # Keadaan apabila data user.csv masih kosong 
    else:
        print("Anda tidak bisa mengubah tipe jin!") # Keadaan apabila bukan Bandung Bondowoso yang mengakses
...
#F09 Laporan Jin
def sumbahan(): # Fungsi untuk menjumlahkan setiap bahan yang terdapat di file bahan_bangunan.csv dan kemudian menyimpannya dalam bentuk array
    sbahan = [0, 0, 0]
...
def sumcandi(): # Fungsi untuk menghitung banyaknya candi yang sudah dibuat
    s = 0
    for i in range(1, data.elemCandi[1]):
...
def laporanjin():
    if data.roleIn == "bandung_bondowoso":
        ...
        if data.elemCandi[1] > 1:
            # Mencari nilai maksimum dan jin terajin
            ...
            # Mencari nilai minimum dan jin termalas
            check = True
        ...
        bah = [0, 0, 0] # Inisialisasi array awal
        for i in range(1, data.elemBahan[1]):
            bah = [bah[j] + int(toArray(data.bahan[i][2])[j]) for j in range(3)] # Ini apa ya aa bagas
